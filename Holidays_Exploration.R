...
calendar_names <- glue("Bank_Holiday_Calendar_{years}.ics") #Saving names of files that will be downloaded
...
#GETTING EQUINOX/SOLSTICE DATAFRAME------------

filepath <- "C:/Users/Pepe/Documents/02 - Master - DS/TFM/"
name <- "astrocal"
extension <- ".ics"
filename <- print(paste0(filepath, name, extension))

seasons_vector1 <- c("Winter","Summer","Autumnal", "Vernal")
seasons_vector2 <- c("Winter","Summer","Autumn", "Spring")

seasons_df <- ical::ical_parse_df(filename) %>%
  select(start,end, summary) %>% 
  mutate_at(vars(summary), funs(as.character(summary))) %>% #converting factor column to characters
  filter(str_detect(summary,"Solstice|Equinox"))


for (i in (1:nrow(seasons_df))) {
  seasons_df[i,"summary"] <- seasons_vector2[str_detect(seasons_df[i,"summary"],seasons_vector1)]
}
...
urls <- glue(url) #Joining url with vector n times
pdf_names <- glue("School_Calendar_{years}.pdf") #Saving the name of the file to be saved with a variable component
...
expand_time_int(df= school_cal_df, start_col="Start", end_col="End", line_col="Line", n_days_col="Num_Days", period=period) %>% 
  arrange(Start) %>% 
  View()
