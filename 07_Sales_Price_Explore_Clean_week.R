...
#Table with registered sales prices. Has some structural problems with data registry
...
campaign_df <- expand_time_int(campaign_df, starting_date_col, ending_date_col, line_col, num_days_col, period=period) %>%
  arrange(No_,desc(Starting_Date_Price_Promo)) %>% #arranging to make inspection easier
  mutate(Period_Price_Promo = Starting_Date_Price_Promo + weeks(Line_Price_Promo-1)) %>% #creating column with date of the promo´s row
...
campaign_df_2 <- expand_time_int(campaign_df_2, starting_date_col, ending_date_col, line_col, num_days_col, period=period) %>% 
  arrange(No_,desc(Starting_Date_Promo)) %>%
  mutate(Period_Promo = Starting_Date_Promo + weeks(Line_Promo-1)) %>%
  mutate(Period = floor_date(Period_Promo,period)) %>% 
  #mutate(Period = floor_date(Period_Promo,period)+1) %>%
  mutate(Period = as_date(Period)) %>% 
  select(Period, everything()) %>% 
...
ssl_df <- query_ssl_copy %>% 
  ...
  group_by(Period = (floor_date(`Shipment Date`,period)+1), Customer_Code, No_) %>%
  summarise(Unit_Price_Sold = max(`Unit Price`)) %>% 
  ungroup() %>% 
  mutate(Period = as_date(Period))

# full join of sales line and sales price, using period as key
full_joined_sales_price <- ssl_df %>% #14532 --OK
...
# stacking full joined (without all duplicates) and unique
