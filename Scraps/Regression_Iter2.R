...
item_df <- period_df %>% 
  filter(Period > as_date('2015-08-01')) %>% #Doing this due to lagged values
...
steps_back <- 12
h=2 #just using a 2 period window because defined functions work better with it. Doesn´t have serious implications
initial <- round(nrow(item_ts)-steps_back)
naive_error <- get_naive_cv_comp(item_ts[,1], h=h, initial=initial)


added_pred <- add_predictors(item_ts, c("Lag_1_Item_Sales"), benchmark=2, h=2)
remaining_pred <- remove_predictors(item_ts[,c("Sum_Quantity",added_pred)], h=2)
...
fc.winners0 %>% autoplot(group=1)


fc.winners0 %>% 
  as.data.frame() %>% 
  mutate(Row = row.names(.)) %>% 
  ggplot(aes(x=seq(20,60))) +
  geom_ribbon(fill="#0953aa", alpha=0.25, aes(ymin = `Lo 80`, ymax = `Hi 80`)) +#+ 
  #geom_ribbon(fill="#00a5cf", alpha=0.25, aes(ymin = Lo_95, ymax = Hi_95)) +
  geom_point(aes(x= Row, y=`Point Forecast`))
  


#All this just to plot the forecast!
past_values <- get(paste0("fc.winners",0))$x %>%
  as.data.frame() %>%
  mutate(Row = as.numeric(rownames(.))) %>% 
  rename (Past = x)

forecast_values <- get(paste0("fc.winners",0)) %>% 
  as.data.frame() %>% 
  mutate(Row= as.numeric(rownames(.)))
...
gg_df <- sales_df_item %>%
  ...
  select(-`Item No_`) %>% 
  create_period_df_month(period = period,months_ahead = 1) %>%
...
gg_df[54,3:ncol(gg_df)]  <- gg_df[54,"Past"]
gg_df[1:53,3] <- NA


gg_df %>%
  ggplot(aes(x=Period)) +
  geom_ribbon(fill="black", alpha=0.25, aes(ymin = Lo_80, ymax = Hi_80)) + 
  geom_ribbon(fill="black", alpha=0.25, aes(ymin = Lo_95, ymax = Hi_95)) +
  geom_line(aes(y=Sum_Quantity, colour ="Actual"), size = 1.8, show.legend = TRUE) +
  geom_line(aes(y=Past), colour="black", size = 0.5, show.legend = TRUE) +
  geom_line(aes(y=Point_Forecast, colour = "Forecast"), size = 1.8, show.legend = TRUE) +
  #my_theme +
