...
#function that creates a lead column with the specified leads
create_lead_weeks <- function(dframe,lead_vector,suffix = "_Item_Sales", period_col=Period, value_col="Sum_Quantity") {
  df_lead <- dframe %>% 
    mutate(Period= Period - weeks(lead_vector[1])) %>% 
    rename(!!paste0("Lead_", lead_vector[1], suffix) := value_col)
  
  if (length(lead_vector)>1) {
    for (i in (2:(length(lead_vector)-1) )) {
      df_lead <- dframe %>%
        mutate(Period= Period + weeks(lead_vector[i])) %>% 
...
# FORECASTING 1st Item---------------------------------------------------------------------

# Approach: 
# Create dataframe with features, where insights from previous exploration are put in to lagged predictors
...
lagged_class_df <- class_sales_df %>% 
  create_lag_weeks(seq(1:10),suffix = "_Class", value_col = "Class_Sales")
...
lagged_item_sales <- filtered_sales %>%
  ...
  create_lag_weeks(c(4,6,8,9,17,18),value_col = "Sum_Quantity")
...
lagged_bank_hol_df <- week_bank_holidays_df %>% 
  ...
  create_lag_weeks(c(1,2,3,4,6),suffix = "_Holidays", value_col = "Num_Holidays")
...
lead_bank_hol_df <- week_bank_holidays_df %>% 
  select(Period, Num_Holidays) %>% 
  create_lead_weeks(c(1,2,3,4),suffix = "_Holidays", value_col = "Num_Holidays")

lead_school_hol_df <- week_school_holidays_df %>% 
  select(Period, Num_School_Holidays) %>% 
  create_lead_weeks(c(1,2,3,4),suffix = "_School_Holidays", value_col = "Num_School_Holidays")
...
item_df <- period_df %>% 
  filter(Period > '2015-02-01') %>% 
  filter(Period < '2019-05-26') %>% 
...
# Iterating adding and removal of predictors, and checking residuals 
...
manual_pred <- append(remaining_pred, c("Lag_4_Item_Sales","Lag_9_Item_Sales" ))
...
manual_pred <-  append(remaining_pred, c("Lag_3_Price_Promo", "Lag_3_Promo", "Lag_3_Holidays"))
added_pred <- add_predictors(item_ts, manual_pred)
remaining_pred <- remove_predictors(item_ts[,c("Sum_Quantity",added_pred)])
...
# -- Set initial dataframe with basic features
# -- Explore relationship of target time series with lagged values of predictors
# -- Set final lagged features based on previous exploration
# -- Hybrid stepwise regression
...
school_holiday_lags <- c(1,2,3,4,7,10,12)
...
bank_holiday_lags <- c(1,2,3,4,6,12)
...
plot_list <- list()
for (i in (1:4)) {
  item_ts_train <- item_ts2[1:(nrow(item_ts)-i*4),] %>% ts()
  item_ts_test <- item_ts2[(end(item_ts_train)[1]+1): (end(item_ts_train)[1]+4),] %>%  ts(start=end(item_ts_train)[1]+1)
